...
export const fetchAreaIdAndName = async (): Promise<AreaIdAndName[]> => {
  const response = await tourServiceHttp.get<AreaIdAndName[]>('/tour-service/api/v1/active-areas/simplified');
  return response.data;
};

export const getActiveAreas = async (page: number | string, limit: number | string, signal?: AbortSignal) => {
  const res = await tourServiceHttp.get<PagedResult<ActiveArea>>('/tour-service/api/v1/active-areas', {
    params: {
      pageSize: limit,
      pageIndex: page
    },
...
export const getActiveArea = async (id: number) => {
  const response = await tourServiceHttp.get<AreaDetailResponse>(`/tour-service/api/v1/active-areas/area-with-guide?areaId=${id}`)
  return response.data
}
