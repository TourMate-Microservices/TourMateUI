...
import { getByAccountId, getTourGuideByAccountId } from "@/api/tour-guide.api";
...
const TourGuideSidebar: FC<TourGuideSidebarProps> = ({ onNavItemClick }) => {
    ...
    if (token) {
        try {
            ...
            accountId = decoded?.AccountId ? decoded.AccountId.toString() : "";
    ...
    React.useEffect(() => {
        if (!accountId) return;
        getByAccountId(accountId as unknown as number).then(setUser).catch(console.error);
    }, [accountId]);
        ...
        <div className="bg-white rounded-2xl shadow-[0_4px_30px_rgba(0,0,0,0.1)] border border-gray-100 p-5 mb-4 md:mb-0 md:mx-0">
            ...
            <div className="flex items-center gap-4 mb-6 p-4 rounded-xl shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                    <h2 className="font-semibold text-lg text-gray-900 truncate">{token ? (accountName || "Người dùng") : "Chưa đăng nhập"}</h2>
                    {token && (
                        <>
